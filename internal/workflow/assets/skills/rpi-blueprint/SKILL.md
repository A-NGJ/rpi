...
On success, suggest → `/rpi-implement <plan-path>` (or, under `--ff`, the chain has already continued there).

## Invariants

- See the project's RPI Skill Contract for `--ff` / `--grill` semantics; both flags apply here and are mutually exclusive
- **Refuse-and-redirect hard gate (runs first, even under `--ff`)**: before writing anything, judge whether the work carries genuine tradeoffs, more than one approach a reasonable engineer would defend, or high blast radius. Apply the split-score *signals* — component count, directory spread, multi-spec — to the problem statement / research as one input; scaffold a throwaway design to score only when the call is genuinely ambiguous. If any hold, decline to produce a plan, explain in one or two sentences why the work needs a reviewable design, and suggest `/rpi-propose <input>`. This gate is an integrity check, not a review pause: it fires even under `--ff` and stops the chain — never silently chain into `/rpi-propose --ff`. If neither a research artifact nor a problem small enough to reason about in one pass is supplied, ask for a research pass or redirect to `/rpi-propose`
...
- Under `--ff`, skip the plan-outline approval pause — write the plan + minimal spec immediately, then invoke `/rpi-implement --ff <plan-path>` via the Skill tool and finally `/rpi-verify <plan-path>` once at the end, propagating `--ff`. The refuse gate above still stops the chain
...
- On success, suggest the next step (`/rpi-implement <plan-path>`), consistent with every other stage
...
- Omit the deliverable, keep the reasoning — `## Design Notes` makes the dropped design auditable so `/rpi-verify` has something to check against
