...
After a successful revision, suggest → `/rpi:rpi-implement <plan-path>`, which resumes from the first unchecked item.
...
- Under `--ff`, after a successful revision invoke `/rpi:rpi-implement --ff <plan-path>` via the Skill tool, resuming from the first unchecked item — but never skip the completed-work assertion
