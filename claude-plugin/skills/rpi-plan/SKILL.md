...
- **Inherit upstream decisions**: in pipeline mode, extend the chain resolution above to collect every non-empty `## Decisions` section from each artifact in the resolved chain (the design and, transitively, the upstream research). Render them in the new plan under a `## Inherited Decisions` heading, each block grouped under `From <source-artifact-path>:` and quoted verbatim, attributed to the artifact that recorded it — the true origin, not the immediate parent or hop depth, so each inherited decision names exactly one source. Keep `## Inherited Decisions` (upstream, carried forward, with source) distinct from the plan's own `## Decisions` (commitments made at this stage). If the chain has no `## Decisions` sections, omit `## Inherited Decisions` entirely — never fabricate an empty or invented block.
...
- **Pre-lock slice audit**: after drafting the phase outline and before the buy-in gate, audit the drafted phases for internal coherence. For a multi-phase plan (or any pipeline plan with a design), spawn the read-only `rpi-slice-audit` subagent via the Task tool over the drafted phases + design path (slice-kind `phases`); it runs the deterministic pre-lock coverage check and adds decision-drift / symbol-mismatch findings. For a single-phase standalone plan, skip the subagent and run only the inline deterministic pre-lock coverage check (one slice cannot forward-reference a sibling or drift from an upstream decision). Surface findings at the buy-in gate: interactive — **blocking**, resolve or waive each before approval; `--ff` — record findings in the plan body and proceed, **unless a hard coverage failure** (a criterion or planned file mapping to no work), which blocks even under `--ff`; `--grill` — run the audit before `grill-me` so grilling operates on already-audited phases. In a split, run the audit per sibling plan, with cross-sibling `**File**:` references checked as coverage.
