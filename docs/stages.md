...
**Inherited decisions:** When a design is proposed from a research note (or any upstream artifact), Propose pulls every `## Decisions` section from the resolved chain (`rpi chain --sections "Decisions"`) and renders them in the new design under an `## Inherited Decisions` heading, grouped by source artifact and quoted verbatim, attributed to the artifact that first recorded each decision. This stays distinct from the design's own `## Decisions` (commitments made at this stage). If the chain records no decisions, no inherited block is added.
...
**Inherited decisions:** In pipeline mode, Plan pulls every `## Decisions` section from the resolved design chain (`rpi chain --sections "Decisions"`, which transitively includes the upstream research) and renders them in the new plan under an `## Inherited Decisions` heading, grouped by source artifact and quoted verbatim. On a multi-hop chain (research → design → plan), each decision is attributed to the artifact that first recorded it, not the intermediate one it passed through -- so a decision made in research surfaces in the plan still pointing at the research note. The block stays distinct from the plan's own `## Decisions`, and absence of upstream decisions yields no block. These inherited decisions are what the decision-drift audit compares each phase against.
